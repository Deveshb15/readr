...
function meta(n: number, over: Partial<ArticleMeta> = {}): ArticleMeta {
  return {
    schema: 1,
    id: ID(n),
...
function memoryFs(opts: { articles?: Record<string, string>; inbox?: string[] } = {}) {
  const articles = new Map(Object.entries(opts.articles ?? {}));
  const inbox = new Set(opts.inbox ?? []);
  const quarantine = new Set<string>();
  let failRemove = false;
  const fs: GroupFs & { inbox: Set<string>; articles: Map<string, string>; quarantine: Set<string>; failRemoves: () => void } = {
    inbox,
    articles,
    quarantine,
